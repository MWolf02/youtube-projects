...
export default function Accordion() { // Defining a functional component named Accordion
  ...
  const buttonText = enableMultiSelection ? "Disable Multiple Selection" : "Enable Multiple Selection"; // Dynamic text for the button based on the enableMultiSelection state

  function handleSingleSelection(getCurrentId) { // Function to handle single item selection
    setSelected(getCurrentId === selected ? null : getCurrentId); // Toggle selected item based on the current state
  }

  function handleMultiSelection(getCurrentId) { // Function to handle multiple item selection
    ...
    if (findIndexOfCurrent === -1) copyMultiple.push(getCurrentId); // If the item is not selected, add it to the array
  ...
  return (
    <div className="wrapper"> {/* Main wrapper for the component */}
      <h1>Project 1</h1> {/* Heading for the project */}
      <button onClick={() => setEnableMultiSelection(!enableMultiSelection)}> {/* Button to toggle multiple selection */}
        {buttonText} {/* Displaying the dynamic text based on the enableMultiSelection state */}
      </button>
      <div className="accordion"> {/* Container for the accordion items */}
        {data && data.length > 0 && data.map((dataItem) => ( // Checking if data exists and is not empty
          <div className="item" key={dataItem.id}> {/* Wrapper for each accordion item */}
            <div
              onClick={
        ...
        {!(data && data.length > 0) && <div>No data found</div>} {/* Displayed when data is empty */}
      </div>
    </div>
  );
}
